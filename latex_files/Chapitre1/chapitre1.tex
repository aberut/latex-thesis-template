...
On peut vouloir automatiser la compilation de ses fichiers latex en PDF.\bigskip

\noindent Si vous utilisez ``GitHub'' pour stocker les sources de votre thèse (ce qui peut être une bonne idée de pour garantir le suivi de votre travail), vous pouvez automatiser la génération des PDF afin qu'elle se fasse à chaque push. Pour ce faire, ce template utilise les ``GitHub Actions''. 
Pour l'utiliser dans votre repo :
\begin{itemize}
\item[$\bullet$] Copiez collez le dossier \textit{.github} à la racine de votre repo (comme ici) 
\item[$\bullet$] Puis dans le fichier \textit{.github/workflows/compile.yml}, vous trouverez plusieurs éléments :
\end{itemize}
Cette variable sert à indiquer l'endroit où sont stockées vos sources .tex :
\begin{lstlisting}
location_latex: "latex_files"
\end{lstlisting}
Ici, ce sont les packages nécessaires à l'installation de l'environnement latex, si des packets en plus sont nécessaires, ajoutez-les à la liste :
\begin{lstlisting}
- name: Install latex
...
\end{lstlisting}
Pour chaque fichier dont vous souhaitez générer un PDF, il vous suffit de créer un bloc comme celui-ci :
\begin{lstlisting}
...
\end{lstlisting}
Enfin ce bloc sert juste à copier les fichiers générés finaux dans un autre dossier (ici le dossier example) :
\begin{lstlisting}
- name: Move examples
  run: |
